...
- **Team Directory** — `/directory` page with grid of all active members: avatar, name, admin badge, email, presence dot + label, one-click Message (opens DM) or Edit my profile (self). Live search by name/email and filters (Everyone/Admins/Members/Active now). Sidebar button `open-directory-button` navigates in. ✅ (Feb 2026)
